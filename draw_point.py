...
class MyApp(QMainWindow):
    ...
    def initUI(self):
        ...
        self.canvas = MyCanvas(self.mainWidget)
        ...
        if os.path.isfile('config.json') == True:
            ...
            try:
                ...
                if 'v_w' in config: self.v_w = config['v_w']
                else: self.v_w = 600
                if 'v_h' in config: self.v_h = config['v_h']
                else: self.v_h = 600
                if 'width' in config: self.top_w = config['width']
                else: self.top_w = 600
                if 'height' in config: self.top_h = config['height']
                else: self.top_h = 600
                if 'canvas_width' in config: self.canvas.w = config['canvas_width']
                else: self.canvas.w = 600
                ...
                if 'show_control_point' in config: 
                    if config['show_control_point'] == 0:
                        ...
                        self.canvas.showControlPoint = True
                else: self.canvas.showControlPoint = True
                if 'show_curve' in config: 
                    if config['show_curve'] == 0:
                        self.canvas.show_curve = False
                    else:
        ...
        self.canvas.v_w = self.v_w
        self.canvas.v_h = self.v_h
        self.canvas.v_org_x = self.v_org_x
        self.canvas.v_org_y = self.v_org_y
        self.canvas.CalcScale()
        self.LoadData()
        ...
        layout1 = QHBoxLayout()
        layoutRightControl = QVBoxLayout()
        self.chkShowControlPoint = QCheckBox("Show Control Point", self)
        self.chkShowControlPoint.setChecked(self.canvas.showControlPoint)
        ...
        layoutRightControl.addWidget(self.chkShowControlPoint)
        self.chkShowCurve = QCheckBox("Show Curve", self)
        self.chkShowCurve.setChecked(self.canvas.show_curve)
        ...
        layoutRightControl.addWidget(self.btnViewUp)
        ...
        layoutRightControl.addWidget(self.btnViewDown)
        ...
        layoutRightControl.addWidget(self.btnViewLeft)
        ...
        layoutRightControl.addWidget(self.btnViewRight)
        ...
        layoutRightControl.addWidget(self.btnZoomIn)
        ...
        layoutRightControl.addWidget(self.btnZoomOut)

        layoutRunMode = QVBoxLayout()
        self.groupBox = QGroupBox("실행모드")
        self.groupBox.setSizePolicy(QSizePolicy(QSizePolicy.Policy.Expanding,QSizePolicy.Policy.Fixed))
        self.groupBox.setLayout(layoutRunMode)
        layoutRightControl.addWidget(self.groupBox)

        self.rdoRunMovePoint = QRadioButton("Move Point", self)
        self.rdoRunMovePoint.clicked.connect(self.rdoMovePoint_clicked)
        ...
        layout1.addLayout(layoutRightControl,10)
    ...
    def resizeEvent(self, event: PySide6.QtGui.QResizeEvent) -> None:
        ...
        print('w, h=%d, %d'%(self.top_w,self.top_h))
    ...
    def closeEvent(self, e):
        ...
        config['v_h'] = self.canvas.v_h
        config['width'] = self.top_w
        config['height'] = self.top_h
        config['canvas_width'] = self.canvas.w
        config['canvas_height'] = self.canvas.h
        config['run_mode'] = self.run_mode
        if self.canvas.showControlPoint:
            config['show_control_point'] = 1
        else:
            config['show_control_point'] = 0
        if self.canvas.show_curve:
            config['show_curve'] = 1
        else:
            config['show_curve'] = 0
    ...
    def findBoundingBox(self):
        ...
        y_min = self.bzPoints[0].y
        x_max = x_min
        y_max = y_min
        for point in self.bzPoints[1:]:
            if x_min > point.x:
                x_min = point.x
            if y_min > point.y:
                y_min = point.y
    ...
    def btnCalcInOut_clicked(self):
        idx = 82
        x1 = self.bzPoints[idx].x
        y1 = self.bzPoints[idx].y
        x2 = self.bzPoints[idx+1].x
        y2 = self.bzPoints[idx+1].y
        x3 = self.bzPoints[idx+2].x
        y3 = self.bzPoints[idx+2].y
        self.lines = []
    ...
    def btnZoomIn_clicked(self):
        self.canvas.zoomIn(self.canvas.width()/2,self.canvas.height()/2)
        self.repaint()
    def btnZoomOut_clicked(self):
        self.canvas.zoomOut(self.canvas.width()/2,self.canvas.height()/2)
        self.repaint()
    ...
    def runMovePointByIdex(self, sel_i):
        ...
        if sel_i == 0:
            sel_prev = len(self.bzPoints)-1
        else:
            sel_prev = sel_i - 1
        x_g = self.bzPoints[sel_prev].x_g2
        y_g = self.bzPoints[sel_prev].y_g2
        dx = x_g - x
        dy = y_g - y
        ...
        if sel_i == len(self.bzPoints)-1:
            ...
            sel_next = sel_i + 1
        x_g = self.bzPoints[sel_next].x_g1
        y_g = self.bzPoints[sel_next].y_g1
        dx = x_g - x
        dy = y_g - y
        l = math.sqrt(dx*dx + dy*dy)
        x_g2_new = x + dx*0.3
        y_g2_new = y + dy*0.3
        self.bzPoints[sel_i].x_g2 = x_g2_new
        self.bzPoints[sel_i].y_g2 = y_g2_new
